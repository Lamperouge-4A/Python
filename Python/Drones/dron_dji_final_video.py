...
def mostrar_video():
    try:
        while True:
            ...
            if frame is None:
                continue  # Evita errores si el frame no está disponible aún
            ...
            texto_bateria = f"Batería: {bateria}%"
            
            # Mostrar la batería en la imagen
            cv2.putText(frame_rgb, texto_bateria, (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 255), 2)
        ...
        print(f"Error en el stream de video: {e}")
    finally:
        tello.streamoff()
        cv2.destroyAllWindows()

# Iniciar el hilo para mostrar el video
video_thread = threading.Thread(target=mostrar_video, daemon=True)
video_thread.start()
